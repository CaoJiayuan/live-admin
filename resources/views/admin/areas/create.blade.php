  ...
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8">
      ...
      <ol class="breadcrumb">
        ...
        <li>
          <a href="/admin/areas">区管列表</a>
        </li>
        <li class="active"><strong>{{ $item ? '修改' : '添加' }}区管</strong></li>
      </ol>
    </div>
    <div class="col-lg-4">
      <div class="title-action">
        <button class="button btn btn-info" type="submit" form="data-form">保存</button>
        <a href="/admin/areas" class="button btn btn-default">返回</a>
      </div>
    </div>
  </div>
@endsection
  ...
  <div class="ibox-content">
    ...
    <form action="{{ url()->current() }}" class="form-horizontal" id="data-form" method="post">
      ...
      <div class="form-group">
        <label class="col-sm-2 control-label" for="data-name">选择区域</label>
        <div class="col-md-5">
          <select class="form-control" id="data-name" name="area_id" autocomplete="off">
            @foreach($areas as $area)
              <option value="{{ $area->id }}"{{ get_form_param('area_id', $item) == $area->id ? 'selected' : '' }}>{{ $area->name }}</option>
            @endforeach
