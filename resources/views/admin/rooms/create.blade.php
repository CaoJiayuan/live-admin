  ...
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8">
      ...
      <ol class="breadcrumb">
        ...
        <li>
          <a href="/admin/rooms">子房管列表</a>
        </li>
        <li class="active"><strong>{{ $item ? '修改' : '添加' }}子房管</strong></li>
      </ol>
    </div>
    <div class="col-lg-4">
      <div class="title-action">
        <button class="button btn btn-info" type="submit" form="data-form">保存</button>
        <a href="/admin/rooms" class="button btn btn-default">返回</a>
      </div>
    </div>
  </div>
@endsection
  ...
  <div class="ibox-content">
    ...
    <form action="{{ url()->current() }}" class="form-horizontal" id="data-form" method="post">
      ...
      <div class="form-group">
        <label class="col-sm-2 control-label" for="data-name">选择房间</label>
        <div class="col-md-5">
          <select class="form-control" id="data-name" name="room_id" autocomplete="off">
            @foreach($rooms as $room)
              <option value="{{ $room->id }}"{{ get_form_param('room_id', $item) == $room->id ? 'selected' : '' }}>{{ $room->title }}</option>
            @endforeach
