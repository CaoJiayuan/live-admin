  ...
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><strong>子房管列表</strong></li>
      </ol>
    </div>
  </div>
@endsection
@section('table:body')
    ...
    <div class="col-md-12">
      <div class="row form-horizontal">
        ...
        <span class="input-group-btn">
            <a type="button" data-toggle="modal" data-target="#data-save" class="btn btn-default add-btn"><i class="fa fa-plus"></i>&nbsp; 添加子房管</a>&nbsp
          </span>
      </div>
    </div>
  </div>
  <div class="ibox-content">
      ...
      @foreach($data as $item)
        <tr>
          ...
          <td>
            ...
            <a class="btn btn-xs btn-info" href="/admin/rooms/{{ $item->id }}/status">{{ $item->enable ? '禁用' : '启用' }}</a>
          </td>
        </tr>
      @endforeach
      </tbody>
    ...
    @endif
    {{ $data->links() }}
    @include("admin.rooms.data-dialog")
  </div>
@endsection
...
@section('js')
  <script>
    ...
    $('.data-edit').click(function () {
      var self = $(this);
      var id = self.data('id');
      $.get('/admin/rooms/'+id+'/edit',function (data) {
        var html = '';
        $('#data-form').data('action','/admin/rooms/'+data.user.id+'/edit');
        $(data.rooms).each(function(index,element){
