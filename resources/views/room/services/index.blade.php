    ...
    <table class="table table-bordered dataTable">
      ...
      @foreach($data as $item)
        <tr>
          ...
          <td>{{ $item->qq }}</td>
          <td>
            <button class="btn btn-xs btn-info data-edit" data-id="{{ $item->id }}" data-name="{{ $item->name }}" data-qq="{{ $item->qq }}"
                    data-toggle="modal" data-target="#data-save">修改
            </button>
            <button class="btn btn-xs btn-danger delete" data-url="/room/services/{{ $item->id }}">删除</button>
          </td>
        </tr>
      @endforeach
      </tbody>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form class="form-horizontal" data-action="/room/services/create" id="data-form">
            {{ csrf_field() }}
            ...
            <div class="form-group">
              <label class="col-sm-2 control-label" for="area-qq">qq</label>
              <div class="col-md-8">
                <input type="text" class="form-control" id="area-qq" name="qq" autocomplete="off" value=""/>
              </div>
            </div>
          </form>
        </div>
