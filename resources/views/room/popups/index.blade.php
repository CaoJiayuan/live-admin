  ...
  <div class="row wrapper border-bottom white-bg page-heading">
    <div class="col-lg-8">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active"><strong>弹窗列表</strong></li>
      </ol>
    </div>
  </div>
@endsection
  ...
  <div class="ibox-content">
    <table class="table table-bordered dataTable">
      ...
      @foreach($data as $item)
        <tr>
          ...
          <td>
            <a href="/room/popups/{{ $item->id }}/status"  class="btn btn-xs btn-info" >{{ !$item->enable ? '显示' : '隐藏' }}</a>
            <button class="btn btn-xs btn-info data-edit" data-id="{{ $item->id }}" data-name="{{ $item->url }}" data-img="{{ $item->img }}"
                    data-toggle="modal" data-target="#data-save">修改
            </button>
            <button class="btn btn-xs btn-danger delete" data-url="/room/popups/{{ $item->id }}">删除</button>
          </td>
        </tr>
      @endforeach
      </tbody>
    ...
    @if($data->total()>=1 && $data->total()<=10)
      <ul class="pagination">
        ...
        <li class="disabled" style="cursor:default">
          <a class="disabled" style="cursor:default" href="javascript:void(0)">总条数:{{$data->total()}}</a>
        </li>
      </ul>
    @endif
    {{ $data->links() }}
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form class="form-horizontal" data-action="/room/popups/create" id="data-form">
            {{ csrf_field() }}
