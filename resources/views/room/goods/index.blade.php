  ...
  <div class="ibox-content">
    <table class="table table-bordered dataTable">
      ...
      <tr>
        ...
        <td>缩略图</td>
        <td class="sorting" data-column="times">兑换次数</td>
      ...
      @foreach($data as $item)
        <tr>
          ...
          <td>
            <a href="/room/goods/{{ $item->id }}/status"  class="btn btn-xs btn-info" >{{ !$item->status ? '显示' : '隐藏' }}</a>
            <button class="btn btn-xs btn-info data-edit" data-id="{{ $item->id }}" data-credits="{{ $item->credits }}" data-img="{{ $item->img }}" data-title="{{ $item->title }}"
                    data-toggle="modal" data-target="#data-save">修改
            </button>
            <button class="btn btn-xs btn-danger delete" data-url="/room/goods/{{ $item->id }}">删除</button>
          </td>
        </tr>
      @endforeach
      </tbody>
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form class="form-horizontal" data-action="/room/goods/create" id="data-form">
            {{ csrf_field() }}
